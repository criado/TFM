...
void prismatoid::cascadeFacets() {
  queue<mask> q; base1=base2=0; numFacets=0; facets=set<mask>();

  for(auto& it: SC) q.push(it.first),
                    base1|= LAYER1 & it.first,
                    base2|= LAYER2 & it.first,
                    numFacets++,
                    facets.insert(it.first);
  ...
  initOptions(); initGraph(); asymmetries=computeAsymmetries();
  #ifdef DEBUG
...
prismatoid::prismatoid(istream& input, rng& gen) {
  SC=map<mask,mask>();
  ...
  memset(permutation, 0, 2*N*sizeof(int));
  permutation[0]=14; permutation[1]=15; permutation[2]=16;
  shuffle(&permutation[0], &permutation[3], gen);
  uniform_int_distribution<int> dis(0, 1);
  int coin= dis(gen);

  permutation[3]= coin? 17: 21;
  permutation[4]= coin? 18: 20;
  permutation[5]= coin? 19: 19;
  permutation[6]= coin? 20: 18;
  permutation[7]= coin? 21: 17;

  for(int i=0; i<8; i++) permutation[permutation[i]]=i;
}
...
void prismatoid::execFlip(flip fl) {
  ...
  mask x=0; do {
    ...
    if(in(f,x)) {
      if(countBits(x)==dim)
        dists.erase(x),--numFacets, facets.erase(x), asymmetries+=symmetriesOf(x);
    ...
    // The supersets of l are appearing faces.
      ...
      if(countBits(x)==dim)
        q.push(x),++numFacets, facets.insert(x), asymmetries-=symmetriesOf(x);
      if(countBits(x)==dim-1) {
...
int prismatoid::symmetriesOf(mask f) {
  mask sym=0; int count=0;
  for(int i=0; i<2*N; i++) {
    if(f&(1<<i)) sym+=(1<<permutation[i]);
  }

  if(facets.count(sym)) count++;
  return count;
}
int prismatoid::computeAsymmetries() {
  int aux=0;
  for(auto& it: facets) aux+=1-symmetriesOf(it);
  return aux;
}

// Number of non-symmetric simplices
double prismatoid::cost() {
  return 1.0;
  //return double(asymmetries);
}
...
bool prismatoid::everythingIsOK() {
  ...
  // 6: symmetries
  // *
  if (asymmetries!=computeAsymmetries()) {
    cout<<numflips<<" milligrams of Uranium"<<endl;
    return false;
  }
